...
export const getIPS = async (req: Request, res: Response) => {
	try {

		const computadoras = await Computadora.find({ internet: true }, 'ip nombreEquipo');
		...
		console.log(error);
		res.status(500).json({
			msg: '[ERROR]: No se pudieron obtener las ips. Contacte con el administrador.'
		})
	}
}
